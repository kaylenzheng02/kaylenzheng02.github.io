(function () {
  ...
  var illustrationLightbox = document.getElementById("illustration-lightbox");
  if (illustrationLightbox) {
    var lbImg = illustrationLightbox.querySelector(".illustration-lightbox__img");
    var lbClose = illustrationLightbox.querySelector(".illustration-lightbox__close");
    var illoGrid = document.querySelector(".site-main__illustration-grid");
    var lastFocus = null;

    function closeIllustrationLightbox() {
      illustrationLightbox.setAttribute("hidden", "");
      if (lbImg) {
    ...
    function openIllustrationLightbox(src, alt) {
      if (!lbImg || !src) return;
      lastFocus = document.activeElement;
      lbImg.src = src;
      lbImg.alt = alt || "";
      illustrationLightbox.removeAttribute("hidden");
      document.body.classList.add("illustration-lightbox-open");
      if (lbClose) lbClose.focus();
    }

    if (illoGrid) {
      illoGrid.addEventListener("click", function (e) {
        var t = e.target;
        if (t.nodeName !== "IMG" || !t.classList.contains("site-main__illustration-img")) return;
        e.preventDefault();
        openIllustrationLightbox(t.currentSrc || t.getAttribute("src") || "", t.getAttribute("alt") || "");
      });
    }

    if (lbClose) {
      lbClose.addEventListener("click", closeIllustrationLightbox);
    }

    illustrationLightbox.addEventListener("click", function (e) {
      if (e.target === illustrationLightbox) {
